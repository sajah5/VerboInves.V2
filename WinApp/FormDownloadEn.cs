...
namespace WinApp
{
    public partial class FormDownloadEn : Form
    {
        private List<string> checkedListData;

        public FormDownloadEn(List<string> checkedListData)
        {
            InitializeComponent();
        ...
        private void SaveAsDocx(string filePath, List<string> selectedFiles)
        {
            ...
            var intro = doc.InsertParagraph()
                .AppendLine("Application name: VerboInves")
                .Font(new Xceed.Words.NET.Font(FontName))
                .FontSize(10)
                .Color(Color.Gray)
                .Italic()
                .SpacingAfter(10)
                .AppendLine("Application version: 3.1")
                .Font(new Xceed.Words.NET.Font(fontName))
                .FontSize(10)
                .Color(Color.Gray)
                .Italic()
                ...
                .Italic()
                .SpacingAfter(10)
                .AppendLine("Release date: 2024/1/30")
                .Font(new Xceed.Words.NET.Font(fontName))
                .FontSize(10)
                .Color(Color.Gray)
                .Italic()
        ...
        private void SaveAsTxt(string filePath, List<string> selectedFiles)
        {
            ...
            {
                // إضافة بيانات المقدمة
                writer.WriteLine("Application name: VerboInves");
                writer.WriteLine("Application version: 3.1");
                writer.WriteLine("Developers:\n" +
                    " Saja Al-Hayek\n," +
                    " Sora Al-Rousan\n," +
                    " Saja Zmaili");
                writer.WriteLine("Release date: 2024/1/30");
                writer.WriteLine("--------------------------------------------------------------------------------");
        ...
        private void SaveAsHtml(string filePath, List<string> selectedFiles)
        {
            using (StreamWriter writer = new StreamWriter(filePath))
            {
                ...
                writer.WriteLine("body { font-family: Georgia, sans-serif; background-color: #f0f0f0; color: #333; margin: 20px; text-align: left; }");
                writer.WriteLine(".intro { font-size: 14px; color: #666; margin-bottom: 10px; text-align: left; }");
                ...
                writer.WriteLine("li { font-size: 16px; margin-bottom: 8px; text-align: ; left}");
                ...
                writer.WriteLine("<p class='intro'>Application name: VerboInves</p>");
                writer.WriteLine("<p class='intro'>Application version: 3.1</p>");
                writer.WriteLine("<p class='intro'>Developers: Saja Al-Hayek,  Sora Al-Rousan, Saja Zmaili</p>");
                writer.WriteLine("<p class='intro'>Release date: 2024/1/30</p>");
        ...
        private void buttonDownload_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (saveFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        MessageBox.Show("Please select files firstا.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void button1_Click_1(object sender, EventArgs e)
        {
            ...
            ReportEn userControl = new ReportEn();
            HideControls();
